...
static uint32_t dmaWrCh_TermianalCnt, dmaWrCh_ErrorCnt;
static uint32_t dmaRdCh_TermianalCnt, dmaRdCh_ErrorCnt;
 ...
 **********************************************************************/
uint8_t MCI_DMA_IRQHandler (void)
{
	uint8_t ret = FALSE;
...
int32_t MCI_Init(uint8_t powerActiveLevel )
{
    ...
    // Set all MCI pins to outputs
			...
			LPC_GPIO1->DIR |= 0x18CC;	// P.15 is NOT SD_PWR 
		#endif
		
    // Force all pins low (except power control pin)
    LPC_GPIO1->CLR = 0x1000;	// P1.12	: D3
    LPC_GPIO1->CLR = 0x0800;	// P1.11	: D2
    LPC_GPIO1->CLR = 0x0080;	// P1.7	: D1
    LPC_GPIO1->CLR = 0x0040;	// P1.6 : D0
  
		#if(POWERMCU_DEV_BOARD == USED_HW)
			LPC_GPIO1->SET = 0x0020;	// P.5 : PWR
		#endif

		LPC_GPIO1->CLR = 0x0008;	// P1.3	: CMD 
    LPC_GPIO1->CLR = 0x0004;	// P1.2 : CLK
