...
)
{
  ...
  SPI_FLASH_CS_LOW();
    ...
    // Write the command
    __LPC17xx_SPI_SendRecvByte(command);
    ...
    if (sys_addr != ADDRESS_NOT_USED)
    {
        switch (command)
        {
            ...
            {
                __LPC17xx_SPI_SendRecvByte((BYTE)((sys_addr >> 16) & 0x000000FF));
                __LPC17xx_SPI_SendRecvByte((BYTE)((sys_addr >>  8) & 0x000000FF));
                __LPC17xx_SPI_SendRecvByte((BYTE) (sys_addr        & 0x000000FF));
                break;
            }
    ...
    switch (command)
    {
        ...
        case SPI_OTPR_CMD:
        {
            ...
            for (data_cycle = 0; data_cycle < Number_Of_Dummy_Bytes; data_cycle++)
                __LPC17xx_SPI_SendRecvByte(0x0);
            break;
        }
        case SPI_DUALIO_HPRD_CMD:
        case SPI_DUALIO_HPRD_4B_CMD:
        ...
        switch (command)
        {
            ...
            case SPI_DUALIO_HPRD_4B_CMD:
            {
                ...
                for (data_cycle = 0; data_cycle < Number_Of_Read_Bytes; data_cycle++)
                    *(data_buffer + data_cycle) = __LPC17xx_SPI_SendRecvByte(0);
                break;
            }
            case SPI_QUADIO_RD_CMD:
            case SPI_QUADIO_RD_4B_CMD:
...
)
{
        ...
        switch (command)
        {
            ...
            case SPI_QPP_4B_CMD:
            {
                ...
                for (data_cycle = 0; data_cycle < Number_Of_Written_Bytes; data_cycle++)
                    __LPC17xx_SPI_SendRecvByte(*(data_buffer + data_cycle));
                break;
            }
            default:
            {
    ...
    SPI_FLASH_CS_HIGH();
#else
	__SEND_CMD_ADR_MULTI_BYTE_RW(command, sys_addr, data_buffer, NULL, Number_Of_Written_Bytes);
#endif
    return(status);
}
