...
static void DrawHB(sSBData_Type *sBarDataPtr);
	  	...
	  	case WM_TIMER: {
			...
			switch (Id) {
				...
				case ID_TIMER_SB_BAT_WARN: {
					sSBData_Type *sBarDataPtr = NULL;
					WM_GetUserData(hWin, &sBarDataPtr, sizeof(sSBData_Type *));
					if(TRUE == sBarDataPtr->BatWarn) {
						if(TRUE == sBarDataPtr->BatVisible) 
							sBarDataPtr->BatVisible = FALSE;
						else sBarDataPtr->BatVisible = TRUE;
  ...
  case WM_PAINT:
	{
		...
		switch(sBarDataPtr->mode) {
			case SB_FULL_MODE: {
				static GUI_RECT gRect;	// Because of its address is given as parameter do it static because it will be 
											// need by emWin later when real drawing is being done // 
				...
				// Draw Volume Level //
				gRect.x0 = SB_STR_COMMON_POSX;
				gRect.x1 = SB_STR_ENDX;
				gRect.y0 = SB_VOLUME_STR_POSY;
				gRect.y1 = gRect.y0 + STATUS_BAR_STR_Y_SZIE;
				sprintf(value_strs[SB_VOLUME], "%02u", sBarDataPtr->Volume);
				GUI_DispStringInRectWrap(value_strs[SB_VOLUME], &gRect, \
					GUI_TA_HCENTER | GUI_TA_VCENTER, GUI_WRAPMODE_WORD);

				// Draw Sensitivity Level //
				gRect.y0 = SB_SENSITIVITY_STR_POSY;
				gRect.y1 = gRect.y0 + STATUS_BAR_STR_Y_SZIE;
				sprintf(value_strs[SB_SENSITIVITY], "%02u", sBarDataPtr->Sensitivity);
				GUI_DispStringInRectWrap(value_strs[SB_SENSITIVITY], &gRect, \
					GUI_TA_HCENTER | GUI_TA_VCENTER, GUI_WRAPMODE_WORD);

				// Draw Brightness Level //
				gRect.y0 = SB_BRIGHTNESS_STR_POSY;
				gRect.y1 = gRect.y0 + STATUS_BAR_STR_Y_SZIE;
				sprintf(value_strs[SB_BRIGHTNESS], "%02u", sBarDataPtr->Brightness);
				GUI_DispStringInRectWrap(value_strs[SB_BRIGHTNESS], &gRect, \
					GUI_TA_HCENTER | GUI_TA_VCENTER, GUI_WRAPMODE_WORD);
				
				// Draw Mineralization (Ground ID) Level // 
				gRect.y0 = SB_GID_STR_POSY;
				gRect.y1 = gRect.y0 + STATUS_BAR_STR_Y_SZIE;
				sprintf(value_strs[SB_GROUND_ID], "%02u", ((((uint32_t)sBarDataPtr->Mineralization)>>8) & 0xFF));
				GUI_DispStringInRectWrap(value_strs[SB_GROUND_ID], &gRect, \
					GUI_TA_HCENTER | GUI_TA_VCENTER, GUI_WRAPMODE_WORD);
				
				// Draw Battery Icon & Level
				#if(1)
				if((FALSE == sBarDataPtr->BatWarn) || \
					((TRUE == sBarDataPtr->BatWarn) && (TRUE == sBarDataPtr->BatVisible))) {
					volatile uint8_t indx;
					struct {
							uint8_t level;
					...
					for(indx=0 ; 0xFF != BatPics[indx].level ; indx++) {
						if(BatPics[indx].level > sBarDataPtr->Battery) {
							indx--;
							break;
						}
						else if(BatPics[indx].level == sBarDataPtr->Battery) {
							break;
						}
					}
					...
					// Draw Battery Level // 
					sprintf(value_strs[SB_BATTERY], "%02u", sBarDataPtr->Battery);
					{
						gRect.y0 = SB_BATTERY_STR_POSY;
						gRect.y1 = gRect.y0 + STATUS_BAR_STR_Y_SZIE;
						GUI_DispStringInRectWrap(value_strs[SB_BATTERY], (GUI_RECT *)&gRect, GUI_TA_HCENTER | GUI_TA_VCENTER, \
							GUI_WRAPMODE_WORD);
					}
				}
				#endif
			...
			case SB_FULL_TOP:
			{
				...
				for(indx=0 ; indx<SB_FULL_TOP_PARt_COUNT ; indx++) {
					...
					if(indx == sBarDataPtr->active_pos)
					...
					switch(indx){
						...
						case AZP_SB_POS_BAT: {
							// Draw Battery Level //
							sprintf(value_strs[SB_BATTERY], "%02u", sBarDataPtr->Battery);
							GUI_DispStringInRectWrap(value_strs[SB_BATTERY], &gRect, \
								GUI_TA_HCENTER | GUI_TA_VCENTER, GUI_WRAPMODE_WORD);
						} break;
						case AZP_SB_POS_GB:
...
void SB_init(uint8_t mode)
{
	...
	switch(mode) {
  	...
  	hWinStatus = WM_CreateWindowAsChild( xpos, ypos, xsize, ysize, \
			WM_HBKWIN, WM_CF_SHOW | WM_CF_MEMDEV, _cbStatus, sizeof(sSBData_Type *));
	// Register callback function to update status bar //
	static sSBData_Type *tptr = &SBData;
	WM_SetUserData(hWinStatus, &tptr, sizeof(sSBData_Type *));
}
...
{ 
	...
	if(0 != hWinStatus)	{ // Check if status-bar is created or not // 
		WM_GetUserData(hWinStatus, &sBarDataPtr, sizeof(sSBData_Type *));
		// Update shared resource and then call_back function will handle rest of operation // 
		if((indx < SB_MEMBER_COUNT) && (NULL != sBarDataPtr)) {
			switch(indx) {
				case SB_SENSITIVITY:			// hassasiyet seviyesi
					if(TRUE == set) sBarDataPtr->Sensitivity = TempSettings.str.Sensitivity	= *((uint16_t *)valptr);
					else *((uint16_t *)valptr) = sBarDataPtr->Sensitivity;
					break;
				case SB_VOLUME:						// Ses seviyesi
					if(TRUE == set) sBarDataPtr->Volume = TempSettings.str.Volume = *((uint16_t *)valptr);
					else *((uint16_t *)valptr) = sBarDataPtr->Volume;
					break;
				case SB_BRIGHTNESS:
					if(TRUE == set) sBarDataPtr->Brightness = TempSettings.str.Brightness = *((uint16_t *)valptr);
					else *((uint16_t *)valptr) = sBarDataPtr->Brightness;
					break;
				case SB_GROUND_ID:
					if(TRUE == set) sBarDataPtr->Mineralization	= TempSettings.str.Mineralization =	*((uint16_t *)valptr);
					else *((uint16_t *)valptr) = sBarDataPtr->Mineralization;
					break;
				case SB_BATTERY:								// Kalan pil seviyesi (% olarak)
					if(TRUE == set) sBarDataPtr->Battery = *((uint16_t *)valptr);
					else *((uint16_t *)valptr) = sBarDataPtr->Battery;
					break;
				case SB_JACK_STATE:
					if(TRUE == set) sBarDataPtr->Jack_State= *((uint16_t *)valptr);
					else *((uint16_t *)valptr) = sBarDataPtr->Jack_State;
					break;
				case SB_AZP_ACTIVE_POS:
					if(TRUE == set) sBarDataPtr->active_pos = *((uint16_t *)valptr);
					else *((uint16_t *)valptr) = sBarDataPtr->active_pos;
					break;
				default:
					while(STALLE_ON_ERR);
					return 0;
		...
		if((indx < SB_MEMBER_COUNT) && (NULL != sBarDataPtr)) {
			switch(indx) {
				case SB_SENSITIVITY:			// hassasiyet seviyesi
					if(TRUE == set) TempSettings.str.Sensitivity	= *((uint16_t *)valptr);
					else *((uint16_t *)valptr) = TempSettings.str.Sensitivity;
					break;
				case SB_VOLUME:						// Ses seviyesi
					if(TRUE == set) TempSettings.str.Volume = *((uint16_t *)valptr);
					else *((uint16_t *)valptr) = TempSettings.str.Volume;
					break;
				case SB_BRIGHTNESS:
					if(TRUE == set) TempSettings.str.Brightness = *((uint16_t *)valptr);
					else *((uint16_t *)valptr) = TempSettings.str.Brightness;
					break;
				case SB_GROUND_ID:
					if(TRUE == set) TempSettings.str.Mineralization =	*((uint16_t *)valptr);
					else *((uint16_t *)valptr) = TempSettings.str.Mineralization;
					break;
				case SB_BATTERY:								// Kalan pil seviyesi (% olarak)
					if(TRUE != set)
						*((uint16_t *)valptr) = TempSettings.str.Battery;
				case SB_JACK_STATE:
						*((uint16_t *)valptr) = HP_JACK_NOT_INSERTED;
					break;
				case SB_AZP_ACTIVE_POS:
					break;
				default:
...
void DrawHB(sSBData_Type *sBarDataPtr)
{
	uint8_t BatLevel = sBarDataPtr->Battery;
	uint16_t x_pos, y_pos = REDUCED_SB_POSY;
	if(SB_REDUCED_MODE_USE_RIGHT == sBarDataPtr->mode)
		x_pos = REDUCED_SB_UR_POSX;
	else if(SB_REDUCED_MODE_USE_LEFT == sBarDataPtr->mode)
		x_pos = REDUCED_SB_UL_POSX;
	else if(SB_REDUCED_MODE_USE_BOTTOM_RIGHT == sBarDataPtr->mode) {
		x_pos = REDUCED_SB_DR_POSX;
		y_pos = REDUCED_SB_DLR_POSY;
	}
	else if(SB_REDUCED_MODE_USE_BOTTOM_LEFT == sBarDataPtr->mode) {
