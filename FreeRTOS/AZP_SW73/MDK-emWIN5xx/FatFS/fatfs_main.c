...
void fatFS_SysTick_Handler_1ms (void) 
{           
...
static void IoInit(void) 
{
	RTC_TIME_Type  current_time;
 	//SysTick_Config(SystemCoreClock/configTICK_RATE_HZ - 1); /* Generate interrupt each 1 ms   */
...
uint8_t sdmmc_ResloadInit(void) {
	uint8_t res = 0;
	IoInit();
	//DEBUGM("%s", mciFsMenu);
	...
	#endif
	// Disc Physical Initialization //
	DEBUGM("rc=%d\n", (res = (uint8_t)disk_initialize((BYTE)0)));
	if(0 != res)
		return res;
	// logical drive initialization (mount) // 
	put_rc((FRESULT)(res = (uint8_t)f_mount((BYTE)0, &Fatfs[0])));
	if(0 != res)
		return res;
...
uint8_t sdmmc_LoadRes(uint8_t ResGroup, ResInfoType *ResInfoPtr)
{
	uint8_t res = 0;
	uint8_t *SDRAMAdr;
	char path[RES_NAME_MAX_LENGTH*2];
	static FIL File1;
	...
	//DEBUGM("Group Indx:%u Name % s\n", ResGroup, ResGroupDirs[ResGroup]);
	//DEBUGM("Group Indx:%u Name % s\n", 21, ResGroupDirs[21]);
	strncpy(path,ResGroupDirs[ResGroup], RES_NAME_MAX_LENGTH);
	path[strlen(ResGroupDirs[ResGroup])] = '/';
	strncpy(&(path[strlen(ResGroupDirs[ResGroup])+1]),ResInfoPtr->name, RES_NAME_MAX_LENGTH);
	put_rc((FRESULT)(res = (uint8_t)f_open(&File1, path, (BYTE)FA_READ)));	// FA_READ disindaki parametreler 
	if(0 != res) {
		ERRM("ResName(%s) fopen() FAILED\n", ResInfoPtr->name);
		goto RES_OPEN_FAILED;
	}
	// Read file fully & store to scratchpad area on SDRAM // 
	{
		uint32_t fsize, fcnt, read;
		for(fsize = File1.fsize, fcnt=0, read=0 ;	fsize != 0 ; fsize -= read, fcnt += read) {
			if(fsize >= blen) read = blen;
			else read = fsize;
			res = f_read(&File1, Buff, read, &read);
			if (res != FR_OK) { // reading FAILED //
				ERRM("ResName(%s) READ ERROR\n", ResInfoPtr->name);
				put_rc((FRESULT)res); 
				break; 
			}	
			if (!read) {	// reading ok but ZERO bytes read //
				ERRM("ResName(%s) ZERO Bytes read\n", ResInfoPtr->name);
				break;
			}	
			memcpy(SDRAMAdr+fcnt, Buff, read);	// Copy read data to SDRAM 
		}
		if(0 != fcnt) {
			TRACEM("ResName(%s): filesize(%d), readbytes(%d BYTEs)\n", ResInfoPtr->name, File1.fsize, fcnt);
			ResInfoPtr->size = fcnt;	// Set file size to resource table member // 
		}
		else
...
/* Terminal will display information */
/* 1. input di 0 to Initialization SD card */
/* 2. input fi 0 to Initialization FATFS system */
/* 3. input fl, you can see all file in SD card */
void main_fatfs (void *pvArg)
{
	char *ptr;
	IoInit();
    xprintf("%s",mciFsMenu);
