...
static void __put_char(unsigned char ch);
static signed char __get_char(void); 
	
#if _USE_XFUNC_IN
	signed char (*xfunc_in)(void);	/* Pointer to the input stream */
	static char *outptr;
#endif
#if _USE_XFUNC_OUT
...
)
{
	...
	i = 0;
	for (;;) {
		while(-1 == (c = xfunc_in()))				/* Get a char from the incoming stream */
