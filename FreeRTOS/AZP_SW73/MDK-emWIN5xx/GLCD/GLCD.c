...
static void GLCD_Config (void)
{
  ...
  GPIO_SetDir(2, (1<<1), 1);     /* Back light enable; define P2.1 as OUTPUT */
	#if(POWERMCU_DEV_BOARD == USED_HW)
		GPIO_SetValue(2, (1<<1) );  
	#elif(UMD_DETECTOR_BOARD == USED_HW)
		GPIO_ClearValue(2, (1<<1) );  
		GPIO_SetDir(1, (1<<5), 1);     /* PWM of LCD Backlight; define P1.5 as OUTPUT temporarly */
		GPIO_SetValue(1, (1<<1) );  
	#endif
	...
	if(0 == buf) {
		for( y_pos = 0; y_pos < GLCD_Y_SIZE; y_pos++ ) {
			for( x_pos = 0; x_pos < GLCD_X_SIZE; x_pos++ )
				GLCD_SetPixel_16bpp_0(x_pos, y_pos, color);
		}
	} else if(1 == buf) {
		for( y_pos = 0; y_pos < GLCD_Y_SIZE; y_pos++ ) {
			for( x_pos = 0; x_pos < GLCD_X_SIZE; x_pos++ )
				GLCD_SetPixel_16bpp_1(x_pos, y_pos, color);
		}
	}	else if(2 == buf) {
		for( y_pos = 0; y_pos < GLCD_Y_SIZE; y_pos++ ) {
			for( x_pos = 0; x_pos < GLCD_X_SIZE; x_pos++ )
				GLCD_SetPixel_16bpp_2(x_pos, y_pos, color);
		}
	}	else if(3 == buf) {
		for( y_pos = 0; y_pos < GLCD_Y_SIZE; y_pos++ ) {
			for( x_pos = 0; x_pos < GLCD_X_SIZE; x_pos++ )
				GLCD_SetPixel_16bpp_3(x_pos, y_pos, color);
		}
	}
}
    ...
    for( i=0; i<16; i++ ) {
        ...
        for( j=0; j<8; j++ ) {
            if( (tmp_char >> 7 - j) & (0x01 == 0x01) ) {
                GLCD_SetPixel_16bpp_0( Xpos + j, Ypos + i, charColor );  /* 字符颜色 */
                //GLCD_SetPixel_16bpp_1( Xpos + j, Ypos + i, charColor );  /* 字符颜色 */
                //GLCD_SetPixel_16bpp_2( Xpos + j, Ypos + i, charColor );  /* 字符颜色 */
                //GLCD_SetPixel_16bpp_3( Xpos + j, Ypos + i, charColor );  /* 字符颜色 */
            }
            else {
                GLCD_SetPixel_16bpp_0( Xpos + j, Ypos + i, bkColor );  /* 背景颜色 */
                //GLCD_SetPixel_16bpp_1( Xpos + j, Ypos + i, bkColor );  /* 背景颜色 */
                //GLCD_SetPixel_16bpp_2( Xpos + j, Ypos + i, bkColor );  /* 背景颜色 */
                //GLCD_SetPixel_16bpp_3( Xpos + j, Ypos + i, bkColor );  /* 背景颜色 */
            }
        }
    }
}
...
void GLCD_DrawLine( uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1 , uint16_t color )
{
	...
	if( dx == 0 ) {
		do { 
				GLCD_SetPixel_16bpp_0(x0, y0, color);
				y0++;
		}
		while( y1 >= y0 ); 
		return; 
