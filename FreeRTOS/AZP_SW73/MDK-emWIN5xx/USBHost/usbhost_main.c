...
int USBHost_main(void)
{
    ...
    xprintf("%s", "Host Initialized\n");
    DEBUGM("%s", "Connect a Mass Storage device\n");
	
    rc = Host_EnumDev();       /* Enumerate the device connected                                            */
    if ((rc == USB_HOST_FUNC_OK) && (Host_GetDeviceType() == MASS_STORAGE_DEVICE)) {
			DEBUGM("Mass Storage device connected\n");
	    ...
	    DEBUGM("Not a Mass Storage device\n");							
      return (0);
    }
...
void Main_ReadDir(uint8_t* pName)
{
    ...
    rc = DIR_Open(pName);	
    DEBUGM("Files/Folders in the directory \"%s\":\n",pName);
    DEBUGM("%3s\t%10s\t%6s\t%s\n","No.","Size","DIR","Name");
	if(rc == 0)
	{
		while(1)
...
static Bool get_objects(void)
{
    ...
    while(1) {
        ...
        if(((pEntry->info.FileAttr & ATTR_DIRECTORY) == 0) && (pEntry->info.FileSize > 0)) {
           file_r = pEntry->name; 
           break;
        }
    }
...
void  Main_Copy (void)
{
    int32_t  fdr;
    int32_t  fdw;
    uint32_t  bytes_read, tot_bytes = 0;
    
    if(!get_objects())
    {
        DEBUGM("No file to copy\n");
        return;
    }
    ...
    if (fdr > 0) {
        ...
        if (fdw > 0) {
            ...
            DEBUGM("Could not open file %s\n", file_w);
            return;
        }
        FILE_Close(fdr);
        DEBUGM("Copy completed\n");
    } else {
        DEBUGM("Could not open file %s\n", file_r);
        return;
    }
