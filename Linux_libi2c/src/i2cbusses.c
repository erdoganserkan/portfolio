...
static enum adt i2c_get_funcs(mj_log_type log_instance, int i2cbus)
{
	unsigned long funcs;
	int file;
	char filename[20];
	...
	file = open_i2c_dev(log_instance, i2cbus, filename, sizeof(filename), 1);
	if (file < 0)
		return adt_unknown;
...
struct i2c_adap *gather_i2c_busses(mj_log_type log_instance)
{
	char s[120];
...
found:
		if (f != NULL) {
			...
			fclose(f);
			if (!px) {
				ERRL(log_instance,  "%s: read error\n", n);
				continue;
			}
			if ((px = strchr(s, '\n')) != NULL)
				*px = 0;
			...
			if (!strncmp(s, "ISA ", 4)) {
				...
				type = i2c_get_funcs(log_instance, i2cbus);
			}
...
static int lookup_i2c_bus_by_name(mj_log_type log_instance, const char *bus_name)
{
	struct i2c_adap *adapters;
	int i, i2cbus = -1;

	adapters = gather_i2c_busses(log_instance);
	if (adapters == NULL) {
		ERRL(log_instance,  "Error: Out of memory!\n");
		return -3;
	}
	...
	if (i2cbus == -1)
		ERRL(log_instance,  "Error: I2C bus name doesn't match any "
			"bus present!\n");
 ...
 */
int lookup_i2c_bus(mj_log_type log_instance, const char *i2cbus_arg)
{
	...
	if (*end || !*i2cbus_arg) {
		...
		return lookup_i2c_bus_by_name(log_instance, i2cbus_arg);
	}
	if (i2cbus > 0xFFFFF) {
		ERRL(log_instance,  "Error: I2C bus out of range!\n");
		return -2;
	}
 ...
 */
int parse_i2c_address(mj_log_type log_instance, const char *address_arg)
{
	long address;
	char *end;
	...
	if (*end || !*address_arg) {
		ERRL(log_instance,  "Error: Chip address is not a number!\n");
		return -1;
	}
	if (address < 0x03 || address > 0x77) {
		ERRL(log_instance,  "Error: Chip address out of range "
			"(0x03-0x77)!\n");
		return -2;
	}
	return address;
...
int open_i2c_dev(mj_log_type log_instance, int i2cbus, char *filename, size_t size, int quiet)
{
	int file;
	...
	if (file < 0 && !quiet) {
		if (errno == ENOENT) {
			ERRL(log_instance,  "Error: Could not open file "
...
int set_slave_addr(mj_log_type log_instance, int file, int address, int force)
{
