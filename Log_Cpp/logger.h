...
#define ERRP(logip, fmt, args...)             \
do{\
    if(logip->is_ok()) \
        logip->Log(ERROR_LEVEL, ERR_S, "" fmt, ##args); \
}while(0);

#define INFOP(logip, fmt, args...)             \
do{\
    if(logip->is_ok()) \
        logip->Log(INFO_LEVEL, INFO_S, "" fmt, ##args); \
}while(0);

#define DEBUGP(logip, fmt, args...)             \
do{\
    if(logip->is_ok()) \
        logip->Log(DEBUG_LEVEL, DBG_S, "" fmt, ##args); \
}while(0);

#define WARNP(logip, fmt, args...)             \
do{\
    if(logip->is_ok()) \
        logip->Log(WARN_LEVEL, WARN_S, "" fmt, ##args); \
}while(0);

#define TRACEP(logip, fmt, args...)             \
do{\
    if(logip->is_ok()) \
        logip->Log(TRACE_LEVEL, TRACE_S, "" fmt, ##args); \
}while(0);
